...
class  Ga_decry:
	def __init__(self,number):
		...
		#message()
	def messe(self):
		file=open('Passwords'+str(self.number),'r')
		paswd=file.readline().split('~')
		passVisual=str(paswd[0])
		passText=str(paswd[1])
		...
		file=open('pit1'+str(self.number)+'.csv','r')
		a=file.readlines()
		cipherLen=len(a)
		pitTable=[]
		...
		time.sleep(1.5)
		objVisual=visual_decrypt(self.number,passVisual)
		...
		for i in range(len(pitTable)):
		    for x in range(width):
		        for y in range(height):
		            if(pitTable[i].getX()==y and pitTable[i].getY()==x):
		                count+=1
		                ...
		                if(decodedIndex==0):
		                    ...
		                    if(pitValue>=0):
		                        decodedValue=arr1[y][x][0] + pitTable[i].getDecodeValue()
		                    else:
		                        decodedValue=pitTable[i].getDecodeValue() + arr1[y][x][0]
		                # If the data is hidden inside the green pixel    
		                elif(decodedIndex==1):
		                    pitValue=pitTable[i].getDecodeValue()
		                    if(pitValue>=0):
		                        decodedValue=arr1[y][x][1] + pitTable[i].getDecodeValue()
		                    else:
		                        decodedValue = pitTable[i].getDecodeValue() + arr1[y][x][1]
		                # If the data is hidden inside the blue pixel
		                elif(decodedIndex==2):
		                    pitValue =  pitTable[i].getDecodeValue()
		                    if(pitValue>=0):
		                        decodedValue = arr1[y][x][2] + pitTable[i].getDecodeValue()
		                    else:
		                        decodedValue = pitTable[i].getDecodeValue() + arr1[y][x][2]
		...
		print("Plain text:",self.message)
		self.mess=plain_text
		return plain_text
		#return self.message
	# def message(self):
	# 	return self.message
